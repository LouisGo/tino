...
- 这里的“本地优先”首先指本地 data plane 与 workflow plane，而不是当前阶段必须先完成本地模型推理
- 当前允许 provider-backed cloud inference，但网络外发必须显式、可控，并带最小本地敏感信息守门
...
- 静默输出应以 digest / thread 汇总为主，不做高频条目级打扰
