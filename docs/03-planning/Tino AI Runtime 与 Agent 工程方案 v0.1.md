...
> 一个建立在真实批次、受控上下文、结构化输出、人类复核和 Rust 副作用边界之上的 AI runtime。
...
职责：管理一个批次从“待处理”到“被接受并落盘”的完整状态流。
...
- `BatchDecisionWorkflow`
  负责对一个 ready batch 做聚类、分类、摘要、置信度判断
...
## 5.6 Layer F: Review / Observability Layer
...
6. `Renderer` 生成 review session，展示到批次审阅面板
7. 用户选择接受 / 改派 / 打回 `_inbox`
8. `Renderer` 将最终审阅结果交给 `Rust command`
9. `Rust` 写入 `topics/` 或 `_inbox/`
...
## 10.4 Phase 3: Human-in-the-loop Review
...
2. 用户能在审阅面板中看见、修改、接受结果
3. 最终结果能被程序稳定落到 `topics/` / `_inbox/`
...
> 先建立一套可控、可审计、可被用户纠正的 AI runtime，再在这套 runtime 上逐步长出更强的 Agent 能力。
