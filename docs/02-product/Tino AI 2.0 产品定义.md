...
> `Tino` 的长期差异化不应建立在“现在和 `NotebookLM` 功能还不一样”上，而应建立在“本地优先 + 系统级接入 + 开放 artifact 闭环”这条结构性路线差异上。
...
- `查 / 研 / 产` 默认由系统隐式判定，只有在歧义会明显影响结果时才追问
...
6. 我不会静默跨 project，也不会把外部知识伪装成 corpus 事实
...
- 静默系统应先聚焦：
  ...
  - resurfacing
  - pending clue
- 静默系统默认遵守不打扰原则：优先在相关上下文里出现，以汇总和重返项目时的接续为主，而不是高频打断
...
> 一个本地优先、系统级接入、开放 artifact 闭环的 `project / corpus` AI 工作台。
